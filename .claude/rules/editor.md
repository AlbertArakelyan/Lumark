...
3. **`isEditorContentSetInitially` gates the initial doc push** from React `content` → CodeMirror. It's reset to `false` when `selectedFile` changes so the new file's content is pushed once, then edits flow back through the `updateListener` without ping-ponging. The reset effect is keyed `[selectedFile]` only — safe today (a folder switch nulls `selectedFile` and unmounts the editor; a folder rename keeps the same bytes open), but a future "move note to another folder" feature must re-key it to `[selectedFolder, selectedFile]`.
