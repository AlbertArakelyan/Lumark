...
3. **`isEditorContentSetInitially` gates the initial doc push** from React `content` → CodeMirror. It's reset to `false` whenever `selectedFile` changes so the new file's content is pushed once, then ongoing edits flow back through the `updateListener` without ping-ponging.
   ...
   That reset effect is keyed on `[selectedFile]` only. This is currently safe: switching folder nulls `selectedFile` (so the whole `Editor` unmounts), and a folder *rename* keeps the same bytes open, so no push is needed. But if a "move note to another folder" feature ever lands — changing `selectedFolder` while `selectedFile` stays put — this effect must become `[selectedFolder, selectedFile]`, or the moved note will keep showing the previous document.
